...
def fmt_amount(val):
    return (
        "••••••"
        if st.session_state.hide_balances
        else f"Rs. {val:,.2f}"
    )
...
eye_label = (
    "👁️ Show Balances"
    if st.session_state.hide_balances
    else "🔒 Hide Balances (Eye)"
)


if st.sidebar.button(eye_label):
    st.session_state.hide_balances = (
        not st.session_state.hide_balances
    )
    st.rerun()
...
if page == "💰 Budget & Envelopes":
    ...
    with st.expander(
        "💳 Main Bank Deposit & Direct Withdrawal"
    ):
        ...
        with sub_c1:
            ...
            if st.button("Add to Bank"):
                ...
                if dep_amt > 0:
                    ...
                    st.success(
                        f"Added Rs. {dep_amt:,.2f} to Main Bank!"
                    )
        ...
        with sub_c2:
            ...
            if st.button("Deduct from Bank"):
                ...
                if with_amt > data["bank_balance"]:
                    ...
                    st.success(
                        f"Deducted Rs. {with_amt:,.2f} from Bank!"
                    )
        ...
        with cols[i % 3]:
            ...
            if b2.button(
                ...
                if amt > (
                    ...
                    st.warning(
                        "Exceeds unallocated bank cash! "
                        "Please deposit to bank first."
                    )
    ...
    with st.expander(
        ...
        new_block_name = nc1.text_input(
            "Category Name "
            "(e.g. Travel, Gym, Education)"
        )


        new_block_amt = nc2.number_input(
            "Starting Allocation (Rs.)",
            min_value=0.0,
            step=500.0
        )
        ...
        if st.button("Create Block"):
            ...
            if new_block_name in data["blocks"]:
                ...
                st.success(
                    f"Created category {new_block_name}!"
                )
...
# ==========================================
    ...
    st.info(
        "Protected space for your website logins "
        "and email accounts."
    )
...
# ==========================================
    ...
    st.title(
        "Groq AI Budget Companion"
    )


    st.write(
        "Ask for financial advice, balance "
        "assessments, or saving strategies "
        "tailored to your allocations."
    )
    ...
    if st.button(
        ...
        if not api_key:
            ...
            st.error(
                "Please provide a Groq API Key to proceed."
            )
            ...
            try:
                ...
                summary = (
                    f"Total balance: "
                    f"{st.session_state.data['bank_balance']}. "
                    f"Allocations: "
                    f"{json.dumps(st.session_state.data['blocks'])}."
                )
                        ...
                        messages=[
                            {
                                ...
                                "content": (
                                    ...
                                    "assistant. Current User "
                                    f"Financial Context: {summary}"
                                )
                            },
                            {
                                "role": "user",
